...
- Start page 的文字策略已調整為「封面入口」：首頁不再放完整遊玩說明，只保留一行氣氛文案與必要權限流程，讓舊紙、四角植物與標題成為第一印象。2026-05-21 起初始畫面也不直接顯示權限 checklist，而是先顯示「行前準備」；點擊後 intro 與主按鈕淡出，再淡入兩個方形勾選框權限按鈕與「等待權限 / 開始探險」。權限 hint 與狀態文字已完全捨棄，準備階段只保留三個按鈕與各自文字，避免第一屏或權限階段像一般 app 權限提示。操作說明移到 Scanning page，首次進入時以置中 modal 顯示，右上角 `×` 關閉；關閉後只留下右上角小型 `?` 指南按鈕，避免長時間遮擋掃描畫面。
