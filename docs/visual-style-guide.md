...
- 最新版本把 rough body 改成 p5.brush 具象頭、胸、腹與分節，Codex 自評約 `7.2/10`，尚待使用者正式評分。

## 目前視覺弱點

- Body 在深綠或暗背景上仍可能被吃掉，仍偏依賴黑色輪廓與少量 highlight。
- Body 已具象化，但整體可能仍偏小，尤其在植物 fixture 中需要注意可讀性。
...
1. 讓 body 在 fixture 背景中更穩定可讀。
2. 做離散 pose preset，讓身體軸線、翅根、前翅、後翅同步連動。
3. 用固定 spawn 與 fixtures 做多 viewport 截圖，確保判讀不被 UI 遮擋。
4. 等使用者評分後再決定是加強 body 具象程度，還是轉向姿態 preset。
