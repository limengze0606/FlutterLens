...
- 目前已採 hybrid UI 架構：`index.html` 內有 DOM app shell，`Pages/DomUi.js` 負責 Start / Scanning / Result 的 DOM 按鈕、toast、頁面 active 狀態與事件接線；p5 canvas 仍負責相機畫面、色彩分析、結果作品圖、昆蟲與 p5.brush。Start page 文字、相機權限按鈕、陀螺儀權限按鈕、開始按鈕、Scanning 的靜態遊玩說明與 shutter、Result 的 Save / Share / Back / toast 已移到 HTML / CSS；尚未加入頁面轉場動畫。
...
- 2026-05-21 Start page 權限流程改成兩段式：初始畫面不顯示權限按鈕，也不再顯示權限 hint，只保留封面文案與「行前準備」。第一次點擊會讓 intro 與主按鈕淡出，接著 fade in 相機 / 陀螺儀權限按鈕與主按鈕；權限未齊時主按鈕為「等待權限」，兩項 granted 後改為「開始探險」。使用者後續要求不需要狀態文字，因此 Start page 準備階段只保留三個按鈕與各自文字。狀態由 `Pages/DomUi.js` 的 `startPreparationRevealed` / `startPreparationTransitionPending` 控制，轉場時間是 `START_PREPARATION_FADE_MS`。
