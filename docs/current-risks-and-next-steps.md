...
- Start 權限流程已改為兩段式加分離式：第一屏只顯示封面文案與「行前準備」，點擊後才淡入「相機權限」與「陀螺儀權限」兩個 checklist 按鈕；兩者同意後主按鈕才從「等待權限」變成「開始探險」。準備階段不顯示 hint 或狀態文字，只保留三個按鈕。2026-05-21 已用 CDP 檢查 DOM 狀態、無狀態文字，以及按「開始探險」後三個按鈕 opacity 變 0；完整 `START -> SCANNING -> RESULT` 仍需在新流程下重跑，真實手機上的相機權限、後鏡頭、HTTPS、DeviceOrientation、觸控手感與效能仍需人工確認。
